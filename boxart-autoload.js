// Build 94: automatically restore previously matched box art while preserving system-art fallbacks.
(()=>{
  if(window.PixelPlayerBoxArtAutoload94)return;window.PixelPlayerBoxArtAutoload94=true;
  ...
  function ensureImage(card){const cover=card.querySelector('.rom-game-cover');if(!cover)return null;let img=[...cover.children].find(x=>x.tagName==='IMG'&&x.classList.contains('pp-boxart-image'));if(!img){img=document.createElement('img');img.className='pp-boxart-image';img.alt='';img.decoding='async';cover.appendChild(img)}return{cover,img}}
  ...
  async function restoreCard(card,map){if(!(card instanceof Element)||!card.classList.contains('rom-game-card'))return;const title=card.querySelector('.rom-game-title')?.textContent?.trim();if(!title)return;const rec=map[title],url=typeof rec==='string'?rec:rec?.url;if(!url||card.querySelector(':scope .rom-game-cover > .pp-boxart-image'))return;const blob=await getBlob(title);if(!blob){setUrl(card,url);return}const pair=ensureImage(card);if(!pair)return;const{cover,img}=pair,obj=URL.createObjectURL(blob);img.onload=()=>{cover.classList.add('has-art');setTimeout(()=>URL.revokeObjectURL(obj),0)};img.onerror=()=>{URL.revokeObjectURL(obj);img.remove();setUrl(card,url)};img.src=obj}
  ...
  const observer=new MutationObserver(records=>{for(const record of records){for(const node of record.addedNodes){if(!(node instanceof Element))continue;if(node.classList?.contains('rom-game-card')||node.querySelector?.('.rom-game-card')){schedule();return}}}});
  observer.observe(list,{childList:true,subtree:true});
  schedule();window.addEventListener('pageshow',schedule);window.addEventListener('pixelplayer:boxart-updated',schedule);
})();