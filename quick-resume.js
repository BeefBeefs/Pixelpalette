// Build 51: invisible per-ROM quick resume slot, separate from visible manual slots.
(()=>{
  ...
  async function waitAndLoad(){for(let i=0;i<180;i++){if(emu()?.gameManager?.FS&&key()){await new Promise(r=>setTimeout(r,350));await quickLoad();return}await new Promise(r=>setTimeout(r,100))}}
