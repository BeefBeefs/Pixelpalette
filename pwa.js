// Build 96: PixelPlayer installable-app bootstrap.
(()=>{
  if(window.PixelPlayerPWA96)return;window.PixelPlayerPWA96=true;
  ...
  if('serviceWorker'in navigator&&location.protocol!=='file:')navigator.serviceWorker.register('coi-serviceworker.min.js?v=96',{scope:'./'}).catch(e=>console.warn('PixelPlayer app worker registration failed',e));
  let deferred=null;
  ...
  const style=document.createElement('style');style.textContent='.pixelplayer-install{margin-left:auto!important;color:#cfff9a!important}.pixelplayer-standalone body{overscroll-behavior:none}.pixelplayer-standalone .tool-tabs{position:sticky;top:0;z-index:1200;backdrop-filter:blur(12px)}.dash-favorites .dash-game-card{grid-template-columns:58px 1fr auto!important}.dash-favorites .dash-game-icon{width:58px!important;height:58px!important;border-radius:13px!important}.dash-favorites .dash-game-icon .system-art-img{max-width:47px!important;max-height:47px!important}@media(max-width:620px){.dash-favorites .dash-game-card{grid-template-columns:54px 1fr auto!important}.dash-favorites .dash-game-icon{width:54px!important;height:54px!important}}';head.appendChild(style);installButton();
})();