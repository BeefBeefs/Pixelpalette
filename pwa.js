// Build 91: PixelPlayer installable-app bootstrap.
(()=>{
  if(window.PixelPlayerPWA91)return;window.PixelPlayerPWA91=true;
  ...
  if('serviceWorker'in navigator&&location.protocol!=='file:')navigator.serviceWorker.register('coi-serviceworker.min.js?v=91',{scope:'./'}).catch(e=>console.warn('PixelPlayer app worker registration failed',e));
  let deferred=null;
  function installButton(){
    const nav=document.querySelector('.tool-tabs');if(!nav||document.getElementById('installPixelPlayerBtn')||standalone())return;
    const b=document.createElement('button');b.id='installPixelPlayerBtn';b.type='button';b.className='tool-tab pixelplayer-install';b.innerHTML='<span class="tab-dot"></span><span>Install App</span>';
    b.hidden=!deferred;b.onclick=async()=>{if(!deferred)return;const p=deferred;deferred=null;b.hidden=true;try{await p.prompt();await p.userChoice}catch{}};nav.appendChild(b)
  }
  ...
  const style=document.createElement('style');style.textContent='.pixelplayer-install{margin-left:auto!important;color:#cfff9a!important}.pixelplayer-standalone body{overscroll-behavior:none}.pixelplayer-standalone .tool-tabs{position:sticky;top:0;z-index:1200;backdrop-filter:blur(12px)}';head.appendChild(style);
  installButton();
})();