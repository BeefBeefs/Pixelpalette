// Build 132: shared session helpers using EmulatorJS native renderer layout with flat diagnostics.
(() => {
  if(window.PixelPlayerSession?.build>=132)return;
  ...
  if(!document.querySelector('link[href*="emulator-performance.css"]')){
    const perfCss=document.createElement('link');perfCss.rel='stylesheet';perfCss.href='emulator-performance.css?v=132';document.head.appendChild(perfCss);
  }

  const compact=document.createElement('style');compact.id='pixelplayer-build132-compact';compact.textContent='.emulator-page .emulator-hero{display:none!important}.emulator-page .tool-tabs{margin-bottom:0}.rom-game-cover{overflow:hidden!important}.rom-game-cover img{width:100%!important;height:100%!important;max-width:none!important;max-height:none!important;object-fit:fill!important;display:block!important;margin:0!important}';document.head.appendChild(compact);
  function inject(src,version=132){if(document.querySelector(`script[src*="${src}"]`))return;const s=document.createElement('script');s.src=`${src}?v=${version}`;s.defer=true;document.body.appendChild(s)}
  inject('version-check.js',132);inject('nav.js',132);inject('pwa.js',132);inject('touch-guard.js',132);inject('core-selector.js',132);inject('folder-scan-progress.js',132);if(page==='gba')inject('gba-zip-support.js',132);inject('folder-indexer-upgrade.js',132);inject('game-art.js',132);inject('boxart-progress.js',132);inject('boxart-autoload.js',132);inject('offline-core-manager.js',132);inject('ui-cleanup.js',132);inject('layout-tweaks.js',132);
  inject('menu-motion.js',132);inject('auto-play-defaults.js',132);inject('direct-recent-launch.js',132);
  inject('controller-themes.js',132);inject('control-layout.js',132);inject('quick-resume.js',132);
  inject('overlay-state-menu.js',132);inject('performance-display.js',132);
  inject('shared-state-thumbnails.js',132);inject('portrait-hitbox-fix.js',132);
  ...
  function cleanStoppedUrl(){const u=new URL(location.href);u.searchParams.delete('play');u.searchParams.delete('resume');u.searchParams.delete('game');u.searchParams.set('build','132');u.searchParams.set('stopped','1');return `${u.pathname}${u.search}${u.hash}`}
  ...
  function rendererReport(){
    ...
    try{gl=canvas?.getContext('webgl2')||canvas?.getContext('webgl');glType=typeof WebGL2RenderingContext!=='undefined'&&gl instanceof WebGL2RenderingContext?'webgl2':gl?'webgl':null;webgl=gl?.getContextAttributes?.()||null;viewport=gl?Array.from(gl.getParameter(gl.VIEWPORT)||[]):null}catch{}
    ...
    window.PixelPlayerPerf={build:132,page,threaded:window.EJS_threads===true,crossOriginIsolated:window.crossOriginIsolated===true,glType,viewport,webgl,canvas:canvas?{bufferW:canvas.width,bufferH:canvas.height,...info(canvas)}:null,canvasParent:info(canvasParent),root:info(root),game:info(game),gamepad:info(pad)};
    console.info('[PixelPlayer Build 132 Renderer JSON] '+JSON.stringify(window.PixelPlayerPerf));
  }
  ...
  window.PixelPlayerSession={build:132,claim,hardNavigate,teardown,stopEmulation,tabId,page,rendererReport};
})();