// Build 96: shared session helpers with Android long-press protection, Play/Games layout refinements, cover-art fallbacks, offline cores and clean stop-emulation behavior.
(() => {
  if(window.PixelPlayerSession?.build>=96)return;
  ...
  const compact=document.createElement('style');compact.id='pixelplayer-build96-compact';compact.textContent='.emulator-page .emulator-hero{display:none!important}.emulator-page .tool-tabs{margin-bottom:0}.pixelplayer-build-id{display:block;margin:6px auto 2px;text-align:center;font:700 9px/1.2 ui-monospace,SFMono-Regular,Consolas,monospace;letter-spacing:.08em;color:#86d957;opacity:.72}.rom-game-cover{overflow:hidden!important}.rom-game-cover img{width:100%!important;height:100%!important;max-width:none!important;max-height:none!important;object-fit:fill!important;display:block!important;margin:0!important}';document.head.appendChild(compact);
  function inject(src,version=96){if(document.querySelector(`script[src*="${src}"]`))return;const s=document.createElement('script');s.src=`${src}?v=${version}`;s.defer=true;document.body.appendChild(s)}
  inject('nav.js',88);inject('pwa.js',96);inject('touch-guard.js',96);inject('core-selector.js',69);inject('folder-scan-progress.js',69);if(page==='gba')inject('gba-zip-support.js',71);inject('folder-indexer-upgrade.js',91);inject('game-art.js',94);inject('boxart-progress.js',86);inject('boxart-autoload.js',94);inject('offline-core-manager.js',92);inject('ui-cleanup.js',93);inject('layout-tweaks.js',95);
  ...
  if(!document.querySelector('.pixelplayer-build-id')){const mark=document.createElement('small');mark.className='pixelplayer-build-id';mark.textContent='BUILD 96';const footer=document.querySelector('footer');if(footer)footer.insertAdjacentElement('afterend',mark);else(document.querySelector('.emulator-shell')||document.body).appendChild(mark)}
  ...
  function cleanStoppedUrl(){const u=new URL(location.href);u.searchParams.delete('play');u.searchParams.delete('resume');u.searchParams.delete('game');u.searchParams.set('build','96');u.searchParams.set('stopped','1');return `${u.pathname}${u.search}${u.hash}`}
  ...
  window.PixelPlayerSession={build:96,claim,hardNavigate,teardown,stopEmulation,tabId,page};
})();