// Build 70: shared session helpers with hard emulator teardown and chunked folder indexing.
(() => {
  if(window.PixelPlayerSession?.build>=70)return;
  ...
  function inject(src,version=70){if(document.querySelector(`script[src*="${src}"]`))return;const s=document.createElement('script');s.src=`${src}?v=${version}`;s.defer=true;document.body.appendChild(s)}
  inject('nav.js',70);inject('core-selector.js',69);inject('folder-scan-progress.js',69);inject('folder-indexer-upgrade.js',70);
  ...
  window.PixelPlayerSession={build:70,claim,hardNavigate,teardown,tabId,page};
})();