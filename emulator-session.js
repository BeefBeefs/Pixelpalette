// Build 71: shared session helpers with hard emulator teardown and folder indexing upgrades.
(() => {
  if(window.PixelPlayerSession?.build>=71)return;
  ...
  function inject(src,version=71){if(document.querySelector(`script[src*="${src}"]`))return;const s=document.createElement('script');s.src=`${src}?v=${version}`;s.defer=true;document.body.appendChild(s)}
  inject('nav.js',71);inject('core-selector.js',69);inject('folder-scan-progress.js',69);if(page==='gba')inject('gba-zip-support.js',71);inject('folder-indexer-upgrade.js',71);
  ...
  window.PixelPlayerSession={build:71,claim,hardNavigate,teardown,tabId,page};
})();