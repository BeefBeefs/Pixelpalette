// Build 89: shared session helpers with clean stop-emulation behavior, installable app shell, full-card artwork fit and visible build marker.
(() => {
  if(window.PixelPlayerSession?.build>=89)return;
  ...
  if(!document.querySelector('.pixelplayer-build-id')){const mark=document.createElement('small');mark.className='pixelplayer-build-id';mark.textContent='BUILD 89';const footer=document.querySelector('footer');if(footer)footer.insertAdjacentElement('afterend',mark);else(document.querySelector('.emulator-shell')||document.body).appendChild(mark)}
  ...
  function cleanStoppedUrl(){
    const u=new URL(location.href);u.searchParams.delete('play');u.searchParams.delete('resume');u.searchParams.delete('game');u.searchParams.set('build','89');u.searchParams.set('stopped','1');return `${u.pathname}${u.search}${u.hash}`;
  }
  function stopEmulation(event){
    event?.preventDefault?.();event?.stopImmediatePropagation?.();
    if(shuttingDown)return;
    shuttingDown=true;
    document.body.classList.remove('rom-playing','pp-transition-in','pp-reveal','low-memory-running');
    teardown('stop-emulation');claimed=false;
    try{localStorage.removeItem(STORAGE_KEY)}catch{}
    requestAnimationFrame(()=>location.replace(cleanStoppedUrl()));
  }
  ...
  window.PixelPlayerSession={build:89,claim,hardNavigate,teardown,stopEmulation,tabId,page};
})();