// Build 88: shared session helpers with installable app shell, full-card artwork fit and visible build marker.
(() => {
  if(window.PixelPlayerSession?.build>=88)return;
  ...
  if(!document.querySelector('.pixelplayer-build-id')){const mark=document.createElement('small');mark.className='pixelplayer-build-id';mark.textContent='BUILD 88';const footer=document.querySelector('footer');if(footer)footer.insertAdjacentElement('afterend',mark);else(document.querySelector('.emulator-shell')||document.body).appendChild(mark)}
  ...
  window.PixelPlayerSession={build:88,claim,hardNavigate,teardown,tabId,page};
})();