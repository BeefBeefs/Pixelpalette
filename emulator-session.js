// Build 98: shared session helpers with complete offline app shell, compositor-safe Android long-press protection, Play/Games layout refinements, cover-art fallbacks, offline cores and clean stop-emulation behavior.
(() => {
  if(window.PixelPlayerSession?.build>=98)return;
  ...
  if(!document.querySelector('.pixelplayer-build-id')){const mark=document.createElement('small');mark.className='pixelplayer-build-id';mark.textContent='BUILD 98';const footer=document.querySelector('footer');if(footer)footer.insertAdjacentElement('afterend',mark);else(document.querySelector('.emulator-shell')||document.body).appendChild(mark)}
  ...
  function cleanStoppedUrl(){const u=new URL(location.href);u.searchParams.delete('play');u.searchParams.delete('resume');u.searchParams.delete('game');u.searchParams.set('build','98');u.searchParams.set('stopped','1');return `${u.pathname}${u.search}${u.hash}`}
  ...
  window.PixelPlayerSession={build:98,claim,hardNavigate,teardown,stopEmulation,tabId,page};
})();