// Build 47: only one PixelPlayer emulator runtime may remain active across browser tabs.
(() => {
  ...
  function inject(src){if(document.querySelector(`script[src*="${src}"]`))return;const s=document.createElement('script');s.src=`${src}?v=47`;s.defer=true;document.body.appendChild(s)}
  inject('nav.js');inject('core-selector.js');
