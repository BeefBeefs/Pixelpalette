// Build 84: shared session helpers with enhanced persistent box-art search.
(() => {
  if(window.PixelPlayerSession?.build>=84)return;
  ...
  const compact=document.createElement('style');compact.id='pixelplayer-build84-compact';compact.textContent='.emulator-page .emulator-hero{display:none!important}.emulator-page .emulator-shell{gap:13px}.emulator-page .tool-tabs{margin-bottom:0}';document.head.appendChild(compact);
  function inject(src,version=84){if(document.querySelector(`script[src*="${src}"]`))return;const s=document.createElement('script');s.src=`${src}?v=${version}`;s.defer=true;document.body.appendChild(s)}
  inject('nav.js',84);inject('core-selector.js',69);inject('folder-scan-progress.js',69);if(page==='gba')inject('gba-zip-support.js',71);inject('folder-indexer-upgrade.js',77);inject('boxart-progress.js',84);
  ...
  window.PixelPlayerSession={build:84,claim,hardNavigate,teardown,tabId,page};
})();