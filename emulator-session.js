// Build 100: shared session helpers + global emulator performance treatment.
(() => {
  if(window.PixelPlayerSession?.build>=100)return;
  ...
  function cleanStoppedUrl(){const u=new URL(location.href);u.searchParams.delete('play');u.searchParams.delete('resume');u.searchParams.delete('game');u.searchParams.set('build','100');u.searchParams.set('stopped','1');return `${u.pathname}${u.search}${u.hash}`}
  ...
  window.addEventListener('pixelplayer:system-ready',()=>{
    ...
    window.PixelPlayerPerf={build:100,page,threaded:window.EJS_threads===true,crossOriginIsolated:window.crossOriginIsolated===true,webgl};
    console.info('[PixelPlayer Build 100]',window.PixelPlayerPerf);
  },{once:true});

  window.PixelPlayerSession={build:100,claim,hardNavigate,teardown,stopEmulation,tabId,page};
})();