// Build 142: shared session helpers using EmulatorJS native renderer layout with flat diagnostics.
(() => {
  if(window.PixelPlayerSession?.build>=142)return;
  ...
  if(!document.querySelector('link[href*="emulator-performance.css"]')){
    const perfCss=document.createElement('link');perfCss.rel='stylesheet';perfCss.href='emulator-performance.css?v=142';document.head.appendChild(perfCss);
  }

  const compact=document.createElement('style');compact.id='pixelplayer-build142-compact';compact.textContent='.emulator-page .emulator-hero{display:none!important}.emulator-page .tool-tabs{margin-bottom:0}.rom-game-cover{overflow:hidden!important}.rom-game-cover img{width:100%!important;height:100%!important;max-width:none!important;max-height:none!important;object-fit:fill!important;display:block!important;margin:0!important}';document.head.appendChild(compact);
  function inject(src,version=142){if(document.querySelector(`script[src*="${src}"]`))return;const s=document.createElement('script');s.src=`${src}?v=${version}`;s.defer=true;document.body.appendChild(s)}
  ...
  function cleanStoppedUrl(){const u=new URL(location.href);u.searchParams.delete('play');u.searchParams.delete('resume');u.searchParams.delete('game');u.searchParams.set('build','142');u.searchParams.set('stopped','1');return `${u.pathname}${u.search}${u.hash}`}
  ...
  window.PixelPlayerSession={build:142,claim,hardNavigate,teardown,stopEmulation,tabId,page,rendererReport};
})();
