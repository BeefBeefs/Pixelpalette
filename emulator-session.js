// Build 126: shared session helpers using EmulatorJS native canvas/thread behavior for compatibility.
(() => {
  if(window.PixelPlayerSession?.build>=126)return;
  ...
  document.body.dataset.build='126';
  ...
  if(!document.querySelector('link[href*="emulator-performance.css"]')){
    const perfCss=document.createElement('link');perfCss.rel='stylesheet';perfCss.href='emulator-performance.css?v=126';document.head.appendChild(perfCss);
  }
  ...
  const compact=document.createElement('style');compact.id='pixelplayer-build126-compact';compact.textContent='.emulator-page .emulator-hero{display:none!important}.emulator-page .tool-tabs{margin-bottom:0}.rom-game-cover{overflow:hidden!important}.rom-game-cover img{width:100%!important;height:100%!important;max-width:none!important;max-height:none!important;object-fit:fill!important;display:block!important;margin:0!important}';document.head.appendChild(compact);
  function inject(src,version=126){if(document.querySelector(`script[src*="${src}"]`))return;const s=document.createElement('script');s.src=`${src}?v=${version}`;s.defer=true;document.body.appendChild(s)}
  inject('version-check.js',126);inject('nav.js',126);inject('pwa.js',126);inject('touch-guard.js',126);inject('core-selector.js',126);inject('folder-scan-progress.js',126);if(page==='gba')inject('gba-zip-support.js',126);inject('folder-indexer-upgrade.js',126);inject('game-art.js',126);inject('boxart-progress.js',126);inject('boxart-autoload.js',126);inject('offline-core-manager.js',126);inject('ui-cleanup.js',126);inject('layout-tweaks.js',126);
  inject('menu-motion.js',126);inject('auto-play-defaults.js',126);inject('direct-recent-launch.js',126);
  inject('controller-themes.js',126);inject('control-layout.js',126);inject('quick-resume.js',126);
  inject('overlay-state-menu.js',126);inject('performance-display.js',126);
  inject('shared-state-thumbnails.js',126);inject('portrait-hitbox-fix.js',126);

  document.querySelectorAll('.pixelplayer-build-id').forEach(x=>x.remove());
  const footer=document.querySelector('footer');
  if(footer)footer.textContent='PixelPlayer • Build 126';
  ...
  function cleanStoppedUrl(){const u=new URL(location.href);u.searchParams.delete('play');u.searchParams.delete('resume');u.searchParams.delete('game');u.searchParams.set('build','126');u.searchParams.set('stopped','1');return `${u.pathname}${u.search}${u.hash}`}
  ...
  window.addEventListener('pixelplayer:system-ready',()=>{
    ...
    window.PixelPlayerPerf={build:126,page,threaded:window.EJS_threads===true,crossOriginIsolated:window.crossOriginIsolated===true,webgl,canvas:canvas?{width:canvas.width,height:canvas.height,clientWidth:canvas.clientWidth,clientHeight:canvas.clientHeight}:null};
    console.info('[PixelPlayer Build 126]',window.PixelPlayerPerf);
  },{once:true});

  window.PixelPlayerSession={build:126,claim,hardNavigate,teardown,stopEmulation,tabId,page};
})();