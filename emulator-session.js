// Build 101: shared session helpers + unified emulator migration runtime.
(() => {
  if(window.PixelPlayerSession?.build>=101)return;
  ...
  document.body.dataset.build='101';
  ...
  if(!document.querySelector('link[href*="emulator-performance.css"]')){
    const perfCss=document.createElement('link');perfCss.rel='stylesheet';perfCss.href='emulator-performance.css?v=101';document.head.appendChild(perfCss);
  }
  ...
  const compact=document.createElement('style');compact.id='pixelplayer-build101-compact';compact.textContent='.emulator-page .emulator-hero{display:none!important}.emulator-page .tool-tabs{margin-bottom:0}.rom-game-cover{overflow:hidden!important}.rom-game-cover img{width:100%!important;height:100%!important;max-width:none!important;max-height:none!important;object-fit:fill!important;display:block!important;margin:0!important}';document.head.appendChild(compact);
  function inject(src,version=101){if(document.querySelector(`script[src*="${src}"]`))return;const s=document.createElement('script');s.src=`${src}?v=${version}`;s.defer=true;document.body.appendChild(s)}
  inject('nav.js',101);inject('pwa.js',100);inject('touch-guard.js',97);inject('core-selector.js',69);inject('folder-scan-progress.js',69);if(page==='gba')inject('gba-zip-support.js',71);inject('folder-indexer-upgrade.js',91);inject('game-art.js',94);inject('boxart-progress.js',86);inject('boxart-autoload.js',94);inject('offline-core-manager.js',92);inject('ui-cleanup.js',93);inject('layout-tweaks.js',95);
  ...
  if(footer)footer.textContent='PixelPlayer • Build 101';
  ...
  function cleanStoppedUrl(){const u=new URL(location.href);u.searchParams.delete('play');u.searchParams.delete('resume');u.searchParams.delete('game');u.searchParams.set('build','101');u.searchParams.set('stopped','1');return `${u.pathname}${u.search}${u.hash}`}
  ...
  window.addEventListener('pixelplayer:system-ready',()=>{
    ...
    window.PixelPlayerPerf={build:101,page,threaded:window.EJS_threads===true,crossOriginIsolated:window.crossOriginIsolated===true,webgl};
    console.info('[PixelPlayer Build 101]',window.PixelPlayerPerf);
  },{once:true});

  window.PixelPlayerSession={build:101,claim,hardNavigate,teardown,stopEmulation,tabId,page};
})();