// Build 54: shared PixelPlayer session helpers and one-active-emulator enforcement.
(() => {
  ...
  function inject(src,version=54){if(document.querySelector(`script[src*="${src}"]`))return;const s=document.createElement('script');s.src=`${src}?v=${version}`;s.defer=true;document.body.appendChild(s)}
  ...
  inject('archive-tools.js',47);inject('controller-themes.js',48);inject('control-layout.js',51);inject('quick-resume.js',53);inject('overlay-state-menu.js',54);
