// Build 74: shared session helpers with hard teardown, folder indexing, and auto play defaults.
(() => {
  if(window.PixelPlayerSession?.build>=74)return;
  ...
  function inject(src,version=74){if(document.querySelector(`script[src*="${src}"]`))return;const s=document.createElement('script');s.src=`${src}?v=${version}`;s.defer=true;document.body.appendChild(s)}
  inject('nav.js',74);inject('core-selector.js',69);inject('folder-scan-progress.js',69);if(page==='gba')inject('gba-zip-support.js',71);inject('folder-indexer-upgrade.js',71);
  ...
  inject('auto-play-defaults.js',74);
  ...
  window.PixelPlayerSession={build:74,claim,hardNavigate,teardown,tabId,page};
})();