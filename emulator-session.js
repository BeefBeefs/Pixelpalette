// Build 48: only one PixelPlayer emulator runtime may remain active across browser tabs.
(() => {
  ...
  function inject(src,version=48){if(document.querySelector(`script[src*="${src}"]`))return;const s=document.createElement('script');s.src=`${src}?v=${version}`;s.defer=true;document.body.appendChild(s)}
  ...
  inject('archive-tools.js',47);inject('controller-themes.js',48);
